...
useSeoMeta({
  title: computed(() => form.value ? `${form.value.title} - SJHAS, Inc.` : 'Form - SJHAS, Inc.'),
})
...
<template>
  <div class="min-h-screen" style="background: linear-gradient(170deg, var(--theme-bg-secondary) 0%, var(--theme-bg-elevated) 40%, var(--theme-bg-secondary) 100%);">
    <!-- Header bar -->
    ...
    <main class="container-narrow section-padding py-12 lg:py-20">
      ...
      <div v-if="isLoading" class="flex items-center justify-center py-32">
        <div class="text-center">
          <Icon name="lucide:loader-2" class="w-8 h-8 animate-spin t-text-accent mx-auto mb-4" />
      ...
      <div v-else-if="error === 'not_found'" class="text-center py-32">
        <div class="w-16 h-16 rounded-2xl t-icon-box mx-auto mb-6 flex items-center justify-center">
          <Icon name="lucide:file-x" class="w-7 h-7" />
        </div>
        <h1 class="text-2xl t-heading t-text tracking-tight mb-3">Form Not Found</h1>
      ...
      <div v-else-if="error === 'error'" class="text-center py-32">
        <div class="w-16 h-16 rounded-2xl bg-red-50 mx-auto mb-6 flex items-center justify-center">
          <Icon name="lucide:alert-circle" class="w-7 h-7 text-red-500" />
        </div>
        <h1 class="text-2xl t-heading t-text tracking-tight mb-3">Something Went Wrong</h1>
        <p class="t-text-secondary mb-8 max-w-md mx-auto leading-relaxed">
      ...
      <div v-else-if="form" class="max-w-2xl mx-auto">
        <div class="t-bg-elevated rounded-3xl t-shadow-lg border t-border p-8 md:p-12">
          <div class="mb-10">
            <h1 class="text-3xl t-heading t-text tracking-tight leading-[1.15] mb-3">
              {{ form.title }}
            </h1>
